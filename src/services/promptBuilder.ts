...
export function buildStage1Prompt(input: Stage1PromptInput): Stage1Prompt {
  ...
  const avoidConcepts = request.regenerate && recentConcepts?.length
  ...
  const user = [
    ...
    request.energy_level ? `Energy level: ${request.energy_level}.` : 'Energy level: not specified.',
    request.curriculum_style ? `Curriculum style: ${request.curriculum_style}.` : 'Curriculum style: not specified.',
